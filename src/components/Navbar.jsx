...
const Navbar = () => {
  ...
  const handleCareersClick     = () => { closeAll(); navigate("/careers"); };
  const handleProductClick     = (name) => { closeAll(); navigate(`/product/${toSlug(name)}`); };
  const handleIndustryClick    = (slug) => { closeAll(); navigate(`/industry/${slug}`); };
  const handleCaseStudyClick   = (id)   => { closeAll(); navigate(`/case-study/${id}`); };
  const handleAboutUsClick     = (scrollTo) => { closeAll(); navigate("/", { state: { scrollTo } }); };
  const handleBrochureClick    = () => closeAll();
  ...
  return (
    <nav ref={navRef} role="navigation" aria-label={t("Navbar.ariaLabel.primaryNavigation")}>
      ...
      <div
        ...
        tabIndex={0}
        aria-label={t("Navbar.ariaLabel.homeLink")}
        onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); closeAll(); navigate("/"); } }}
      >
        <img src={logo} alt={t("Navbar.CVITLogo")} />
      </div>
